...
const UpdateProblem = () => {
  ...
  useEffect(() => {
    const fetchProblems = async () => {
      try {
        setLoading(true);
        // Note: You should generally handle pagination better than limit=100
  ...
  const filteredProblems = problems.filter(
    (problem) =>
      ...
      // Assuming 'tags' is a string. If it's an array, you need to stringify/check elements.
  ...
  // ✅ NEW HANDLER: Pass problem details via navigation state
  const handleAddVideoClick = (problem) => {
    navigate(`/video/problem/${problem._id}`, {
      state: {
        problemId: problem._id,
        problemTitle: problem.title,
        // You can add more fields if needed, like problem.description
      },
    });
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-zinc-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Success/Error Messages (Omitted for brevity, but keep them) */}
        {/* ... (Success/Error Message JSX) ... */}
        {success && (
          <motion.div
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center space-x-3 ml-6">
                    ...
                    <button
                      // 🔄 MODIFIED: Use the new handler to pass state data
                      onClick={() => handleAddVideoClick(problem)}
                      className="flex items-center space-x-2 px-3 py-2 text-yellow-400 hover:text-amber-300 hover:bg-amber-900/20 rounded-lg transition-colors"
                    >
