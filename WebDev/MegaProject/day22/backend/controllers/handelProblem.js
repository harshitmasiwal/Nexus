...
const updateProblem = async (req, res) => {
  try {
    ...
    // console.log(data)
      ...
      for (const { language, completeCode } of data.referenceSolution) {
        const languageId = getLanguageById(language);
        ...
        for (const { input, output } of data.visibleTestCases) {
         ...
         for (const { input, output } of data.hiddenTestCases) {
          const obj = {
            source_code: completeCode,
            language_id: languageId,
            stdin: input,
...
const getAllProblems = async (req, res) => {
  try {
    ...
    const file = await Problem.find({}).skip(skip).limit(limit).select(['_id' ,'title' , 'difficulty' ,'tags' , 'createdAt' , 'updatedAt']);
...
const getAllProblemSolvedByUser = async (req,res) => {
  try{
    ...
    const reply = user.problemSolved
    res.json(reply)
