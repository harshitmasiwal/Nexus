// // import { Menu, X, Home, User, Briefcase, Mail, Rocket, LogOut , UserStar } from 'lucide-react';
...
// //   const links = [
// //     isAuthenticated && props?.role === 'admin'
// //       ? { name: 'Admin Panel', icon: UserStar, onClick: () => navigate('/admin') }
// //       : null,
...
// //   return links;
// // }, [isAuthenticated, props?.role, navigate]);
...
// import { Menu, X, User, Rocket, LogOut , UserStar } from 'lucide-react';
// import { useState, useEffect, useRef, useMemo } from 'react';
// import { useDispatch, useSelector } from 'react-redux';
// import { logoutUser } from '../store/slices/authSlice';
// import { useNavigate } from 'react-router';
...
const Navbar = () => {
  ...
  const BrandLogo = ({ textSize = "text-xl", iconSize = "w-6 h-6" }) => (
    <div className="flex items-center space-x-2">
      <Rocket className={`${iconSize} text-amber-500 transform hover:rotate-6 transition-transform duration-500`} />
      <span className={`${textSize} font-extrabold text-gray-50`}>
        Code Masti
      </span>
    </div>
  );
