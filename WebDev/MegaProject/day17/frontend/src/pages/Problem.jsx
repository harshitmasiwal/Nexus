...
const Problem = () => {
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            ...
            const data = response.data;
            setResult(data);
            // FIX: Use data.status === 'accepted' check for success
    ...
    const renderResultContent = () => {
        ...
        // Successful Run or Accepted Submit
        // FIX: Check for result.status === 'accepted' for submission success
        if (isRun && result.success || !isRun && result.status === 'accepted') { 
             const statusMessage = isRun ? "✅ All visible test cases passed!" : "🎉 Accepted!";
             const testCases = isRun ? result.testCases : [];
