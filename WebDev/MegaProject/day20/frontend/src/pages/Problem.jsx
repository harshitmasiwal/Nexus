...
import { ChevronDown, BookOpen, Terminal, CheckCircle, XCircle, FileText, FlaskConical, History, GripVertical, GripHorizontal, MessageCircle, PanelLeft, PanelRight, ChevronLeft, ChevronRight, Maximize2, Minimize2 } from 'lucide-react';
...
const DifficultyPill = ({ difficulty }) => {
    ...
    if (difficulty === 'Easy') colorClass = 'bg-green-700/80 text-green-100 border border-green-500/30';
    if (difficulty === 'Medium') colorClass = 'bg-yellow-700/80 text-yellow-100 border border-yellow-500/30';
    if (difficulty === 'Hard') colorClass = 'bg-red-700/80 text-red-100 border border-red-500/30';

    return (
        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium shadow-lg transition-all duration-300 hover:scale-105 ${colorClass}`}>
            {difficulty}
        </span>
    );
};
...
const TestCaseDisplay = ({ testCase, index, isActive, onClick }) => (
    <div
        className={`p-4 rounded-lg cursor-pointer transition-all duration-300 ease-in-out transform hover:scale-[1.02] ${
            isActive ? 'bg-zinc-700/80 border-l-4 border-yellow-500 shadow-xl shadow-yellow-500/10 backdrop-blur-sm' : 'bg-zinc-800/60 hover:bg-zinc-700/60 backdrop-blur-sm'
        }`}
        ...
        {isActive && (
            <div className="mt-4 space-y-3 text-sm animate-in slide-in-from-top-2 duration-300">
                <div className="p-3 bg-zinc-900/80 rounded-lg border border-zinc-700/50">
                    ...
                    <pre className="text-white whitespace-pre-wrap break-all font-mono text-sm">{testCase.input}</pre>
                </div>
                <div className="p-3 bg-zinc-900/80 rounded-lg border border-zinc-700/50">
                    <p className="text-zinc-400 font-mono text-xs mb-1">Output:</p>
                    <pre className="text-white whitespace-pre-wrap break-all font-mono text-sm">{testCase.output}</pre>
                </div>
                {testCase.explanation && (
                    <div className="p-3 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                        <p className="text-zinc-400">
                            <span className="font-semibold text-blue-300">Explanation:</span> {testCase.explanation}
                    </p>
                    </div>
                )}
            </div>
...
const Problem = () => {
    ...
    function base64ToText(base64) {
        try {
            ...
            const nonPrintableCount = (decoded.match(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/g) || []).length;
    ...
    const handleRunCode = async () => {
        ...
        try {
            ...
            const errorData = {
                status: 'error',
                errorMessage: error.response?.data?.errorMessage || error.response?.data?.error || `Request failed. Check network or server status: ${error.message}`,
                testCasesPassed: 0,
                runtime: 0,
                memory: 0
            };
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            ...
            const errorData = {
                status: 'error',
                errorMessage: error.response?.data?.errorMessage || error.response?.data?.error || `Submission API failed. Check connectivity or endpoint: ${error.message}`,
                testCasesPassed: 0,
                runtime: 0,
                memory: 0
            };
            ...
            return (
                <div className="bg-zinc-800/60 rounded-md p-3">
                    <pre className="text-xs font-mono text-zinc-300 text-center">
                        {runStatus === 'running' ? 'Processing submission...' : 'Loading...'}
                    </pre>
                </div>
            );
        }
        ...
        return (
            <div className="space-y-2">
                ...
                {result.testCases.map((testCase, index) => {
                    ...
                    return (
                        <div key={index} className="bg-zinc-800/60 rounded-md p-2 space-y-2">
                            <div className="flex items-center justify-between">
                                <h4 className="text-xs font-semibold text-zinc-200">Test Case {index + 1}</h4>
    ...
    return (
        <div className={`font-sans antialiased text-zinc-100 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 flex h-screen overflow-hidden ${isMobile ? 'flex-col' : ''}`}>
            {/* Party Animation */}
            <PartyAnimation 
            ...
            <PanelGroup direction={isMobile ? "vertical" : "horizontal"}>
                ...
                <Panel 
                    ...
                    className={`flex flex-col transition-all duration-300 ease-in-out bg-zinc-900/50 backdrop-blur-sm ${!isLeftPanelExpanded && 'hidden'}`}
                ...
                >
                    <div className="flex-shrink-0 flex items-center justify-between border-b border-zinc-700/50 bg-zinc-800/80 backdrop-blur-lg">
                            ...
                            {tabs.map(tab => {
                                ...
                                return (
                                    <button key={tab.key} onClick={() => setActiveLeftTab(tab.key)}
                                        ...
                                        ${activeLeftTab === tab.key ? 'text-white bg-yellow-500/20 border border-yellow-500/30 shadow-lg shadow-yellow-500/10' : 'text-zinc-400 hover:bg-zinc-700/50 hover:text-zinc-200'}`}>
                                        <Icon className={`w-4 h-4 transition-transform duration-300 ${activeLeftTab === tab.key ? 'scale-110' : 'group-hover:scale-105'}`} />
                                        <span className="transition-all duration-300 text-xs">{tab.name}</span>
                                    </button>
                                );
                            })}
                        </div>
                        <button
                            ...
                            title={isLeftPanelExpanded ? "Collapse left panel" : "Expand left panel"}
                        >
                            <ChevronLeft className={`w-5 h-5 transition-transform duration-200 ${isLeftPanelExpanded ? 'rotate-0' : 'rotate-180'} group-hover:scale-110`} />
                        </button>
                    </div>
                    <div className="flex-grow p-6 overflow-y-auto left-panel-scrollbar bg-gradient-to-b from-transparent to-zinc-900/20">
                            ...
                            <div className="mb-6 p-4 bg-gradient-to-r from-zinc-800/90 to-zinc-700/80 backdrop-blur-sm rounded-lg border border-zinc-700/50 shadow-lg">
                                <div className="flex items-center space-x-2 mb-3">
                                    ...
                                    <div className={`ml-auto px-2 py-1 rounded-full text-xs font-medium ${
                                        executionSummary.status === 'accepted' ? 'bg-green-600/20 text-green-400 border border-green-500/30' : 'bg-red-600/20 text-red-400 border border-red-500/30'
                                    }`}>
                                        {executionSummary.status === 'accepted' ? '✓ Passed' : '✗ Failed'}
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-3 text-sm">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className={`font-medium px-2 py-1 rounded ${
                                            executionSummary.status === 'accepted' ? 'bg-green-600/20 text-green-400' : 'bg-red-600/20 text-red-400'
                                        }`}>
                                            {executionSummary.testCasesPassed}/{executionSummary.testCaseTotal}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-zinc-400">Runtime:</span>
                                        <span className="text-zinc-200 font-medium bg-zinc-700/50 px-2 py-1 rounded">
                                            {executionSummary.runtime ? `${executionSummary.runtime}s` : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-zinc-400">Memory:</span>
                                        <span className="text-zinc-200 font-medium bg-zinc-700/50 px-2 py-1 rounded">
                                            {executionSummary.memory ? `${executionSummary.memory} KB` : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center">
                        ...
                        {activeLeftTab === 'description' && (
                            <div className="space-y-6 animate-in fade-in duration-500">
                                <h1 className="text-3xl font-bold bg-gradient-to-r from-white to-zinc-300 bg-clip-text text-transparent">{problem.title}</h1>
                                <div className="flex items-center gap-3 flex-wrap">
                                    <DifficultyPill difficulty={problem.difficulty} />
                                    {problem.tags && problem.tags.split(',').map((tag, i) => (
                                        <span key={i} className="px-3 py-1 text-sm rounded-full bg-blue-900/60 text-blue-300 font-medium border border-blue-500/30 shadow-lg hover:scale-105 transition-all duration-300">{tag.trim()}</span>
                                    ))}
                                </div>
                                <div className="prose prose-invert max-w-none">
                                    <p className="text-zinc-300 leading-relaxed whitespace-pre-wrap text-lg" dangerouslySetInnerHTML={{ __html: problem.description.replace(/\n/g, '<br/>') }} />
                                </div>
                ...
                <Panel defaultSize={isMobile ? 60 : 60} minSize={30} maxSize={100} className="flex flex-col transition-all duration-300 ease-in-out bg-zinc-900/50 backdrop-blur-sm">
                    <div className="flex-shrink-0 flex items-center justify-between p-3 px-10 border-b border-zinc-700/50 bg-zinc-800/80 backdrop-blur-lg">
                        ...
                        <div className="space-x-3">
                            <button 
                                ...
                                className={`px-4 py-2 text-sm font-semibold rounded-lg shadow-lg transition-all duration-200 transform hover:scale-105 ${(isSubmitting || buttonsDisabled) ? 'bg-zinc-600 cursor-not-allowed' : 'bg-yellow-600 hover:bg-yellow-500 hover:shadow-yellow-500/25'}`}
                            >
                                {buttonsDisabled ? `Run Code (${buttonDisableTimer}s)` : 'Run Code'}
                            </button>
                            <button 
                                onClick={handleSubmitCode} 
                                disabled={isSubmitting || buttonsDisabled} 
                                className={`px-4 py-2 text-sm font-semibold rounded-lg shadow-lg transition-all duration-200 transform hover:scale-105 ${(isSubmitting || buttonsDisabled) ? 'bg-zinc-600 cursor-not-allowed' : 'bg-green-600 hover:bg-green-500 hover:shadow-green-500/25'}`}
                            >
                    ...
                    <PanelGroup direction="vertical">
                        ...
                        <Panel 
                            ...
                            className={`flex-shrink-0 bg-zinc-800/80 backdrop-blur-lg border-t border-zinc-700/50 transition-all duration-300 ease-in-out flex flex-col ${!isConsoleExpanded && 'hidden'}`}
                        >
                            ...
                            <div className="flex-shrink-0 flex items-center justify-between p-3 border-b border-zinc-700/50">
                                <div className="flex items-center space-x-2">
                                    ...
                                    {runStatus && (
                                        <div className={`flex items-center space-x-1 ml-4 px-2 py-1 rounded-full text-xs font-medium transition-all duration-300 ${runStatus === 'success' ? 'bg-green-600 text-white shadow-lg shadow-green-500/25' : runStatus === 'error' ? 'bg-red-600 text-white shadow-lg shadow-red-500/25' : 'bg-blue-600 text-white shadow-lg shadow-blue-500/25'}`}>
                                ...
                                >
                                    <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isConsoleExpanded ? 'rotate-0' : 'rotate-180'} group-hover:scale-110`} />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
                                {isSubmitting && runStatus === 'running' ? (
                                    <div className="flex items-center space-x-2 text-zinc-300">
                                        ...
                                        <pre className="text-xs font-mono whitespace-pre-wrap break-all">Executing your code...</pre>
                                    </div>
                                ) : (
                                    renderResultContent()
                                )}
            ...
            <button
                ...
                className="fixed bottom-6 left-6 w-14 h-14 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white rounded-full shadow-lg hover:shadow-xl hover:shadow-blue-500/25 transition-all duration-300 flex items-center justify-center z-40 group"
            >
            ...
            {showAIChat && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-zinc-900/95 backdrop-blur-lg rounded-xl border border-zinc-700/50 w-full max-w-4xl max-h-[80vh] flex flex-col shadow-2xl shadow-black/50">
                        <div className="flex items-center justify-between p-4 border-b border-zinc-700/50 bg-zinc-800/50">
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="ml-4">
                                    ...
                                    <p className="text-xs text-zinc-400">Ask me anything about this problem</p>
                                </div>
                            </div>
                            <button
