...
app.use(express.json({
    strict : true
})) //ye hamare req.body mai aane wale data ko parse karta hai in json

app.use(cookieParser()) //for parsing the data recived in cookie in json 
...
app.get('/user',async (req,res)=>{
    ...
    try{
        ...
        const user = await Citizens.findOne({email:payload.email})
        res.send(user)
    }
    catch(err){
        res.send("Error : "+err.message)
